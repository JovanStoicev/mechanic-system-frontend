...
export default function JobsListPage() {
  ...
  return <div>
    ...
    <div className="mt-4 overflow-x-auto rounded-xl border"><table className="w-full text-sm"><thead className="bg-slate-50 text-slate-600"><tr><th className="px-4 py-3 text-left">Car</th><th className="px-4 py-3 text-left">Mechanic</th><th className="px-4 py-3 text-left">Description</th><th className="px-4 py-3 text-left">Labour</th><th className="px-4 py-3 text-left">Total</th><th className="px-4 py-3 text-left">Status</th><th className="px-4 py-3 text-left">Action</th></tr></thead>
      <tbody>{jobs.map((job) => <tr key={job.id} className="border-t"><td className="px-4 py-3"><div>{job.carBrand} {job.carModel}</div><div className="font-mono text-xs text-slate-500">{job.vin}</div></td><td className="px-4 py-3">{job.mechanicName}</td><td className="px-4 py-3">{job.description}</td><td className="px-4 py-3">€{job.labourCost.toFixed(2)}</td><td className="px-4 py-3">€{job.totalCost.toFixed(2)}</td><td className="px-4 py-3">{job.status}</td><td className="px-4 py-3"><Link className="rounded-lg border px-3 py-1" to={`/boss/jobs/${job.id}`}>View</Link></td></tr>)}</tbody>
    </table></div>
  </div>;
}
