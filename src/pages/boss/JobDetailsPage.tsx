...
export default function JobDetailsPage() {
  ...
  return <div>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <section className="rounded-xl border p-4"><h2 className="font-semibold">Pricing</h2><dl className="mt-2 space-y-2 text-sm"><div className="flex justify-between"><dt>Parts</dt><dd>€{job.partsCost.toFixed(2)}</dd></div><div className="flex justify-between"><dt>Labour</dt><dd>€{job.labourCost.toFixed(2)}</dd></div><div className="flex justify-between border-t pt-2 font-semibold"><dt>Total</dt><dd>€{job.totalCost.toFixed(2)}</dd></div><div className="flex justify-between"><dt>Mechanic bonus (10%)</dt><dd>€{(job.labourCost * 0.1).toFixed(2)}</dd></div></dl><p className="mt-4 text-sm">Status: <strong>{job.status}</strong></p>{job.completedAt && <p className="mt-1 text-xs text-slate-500">Completed {new Date(job.completedAt).toLocaleString()}</p>}</section>
    </div>
