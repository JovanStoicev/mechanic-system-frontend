...
export type PartRequestStatus = "PENDING" | "APPROVED" | "REJECTED" | "RECEIVED";
export type PartRequest = { id: number; mechanicId: number; mechanicName: string; partId: number; partName: string; quantity: number; note: string | null; status: PartRequestStatus; createdAt: string; updatedAt: string | null };
...
export const receivePartRequest = (id: number) => api<PartRequest>(`/api/boss/part-requests/${id}/receive`, { method: "PATCH" });
